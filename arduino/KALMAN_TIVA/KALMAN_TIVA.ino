...
#define VDD 3300.0f       //Analog reference voltage in milivolts
...
void setup() {
  ...
  for(i=0;i<=2;i++){                  // X,Y,Z axis
    config.zeroLevel[i] = 1500;       // Accelerometer zero level (mV) @ 0 G
    config.inpSens[i] = 300;          // Accelerometer Sensisitivity mV/g
  }        
  
  for(i=3;i<=4;i++){
    config.inpSens[i] = 1200;	    // Gyro Sensitivity mV/deg/ms    
...
float getInput(char i){
  ...
  tmpf = an[i] * VDD / 4095.0f;  //voltage (mV)
